    ...
    canvas.toBlob(
      async (blob) => {
        ...
        try {
          ...
          // Show loading state
          const loadingAlert = `Processing image${
            locationData ? " with GPS coordinates" : " (no GPS available)"
          }...`;
            ...
            if (data.is_official_metro_sign) {
              if (data.validation) {
                ...
                if (validation.isValid && validation.matchedStation) {
                  ...
                  const lines = stationFeatures
                    .map((feature) => {
                      const rescom = feature.properties?.res_com || "";
                      const lineColor =
                        feature.properties?.colourweb_hexa || "666666";
                ...
                showNotification({
                  ...
                  message:
                    "Official metro sign found but could not validate station",
                  details: {
              ...
              showNotification({
                ...
                message:
                  "The image doesn't appear to show an official RATP metro station sign.",
                details: {
